...
#
# Amazon University: interceptor-first (DOM fallback) scraper on the
# amazon.jobs studentprograms search SPA. The legacy search.json API ignores
# the Intern/US/category facets, so the JS-rendered search is the only source.
#
...
def scrape() -> list[Job]:
    ...
    with sync_playwright() as pw:
        ...
        if PLAYWRIGHT_JOBS_ENABLED.get("amazon_university"):
            try:
                jobs.extend(_scrape_amazon_university(page))
            except Exception as exc:
                print(f"[playwright_jobs/amazon_university] WARNING: {exc}")
...
def _scrape_intern_list(page) -> list[Job]:
    ...
    for embed_url in _EMBED_URLS:
        # Canadian postings get a "[CANADA] " company prefix so they stand out
        # in the inbox — the email subject is "🚀 {company} — {title} ({loc})",
        # so prefixing the company puts "[CANADA]" right up front, before the
        # company name, with no emailer/schema changes. US postings (the
        # majority) are unprefixed.
        is_canada = "/ca/" in embed_url
...
def _scrape_amazon_university(page) -> list[Job]:
    """
    Scrape Amazon's university / student-programs internships.

    The marketing landing page
      amazon.jobs/content/en/career-programs/university/internships-for-students
    renders no postings — its "Find open internships" CTA points at the SPA
      amazon.jobs/en/search?...&category_type=studentprograms
    which is JavaScript-rendered. The legacy search.json API used by
    big_tech.py::_scrape_amazon() silently ignores the category_type / country /
    employment-type facets (any value returns the full ~10k global job set), so
    a stdlib JSON scraper on those filters is not possible — hence Playwright.

    Strategy (interceptor-first, DOM fallback — same shape as _scrape_uber +
    _scrape_apple):
      - Response interceptor heuristically captures any amazon.jobs JSON whose
        body contains a list of dicts with a "title" field. Robust against the
        obfuscated DOM.
      - DOM fallback parses rendered job-detail anchors (/en/jobs/{id}/...) when
        nothing was intercepted on a given page.
      - Paginates by incrementing the `offset` query param; stops when a page
        yields no new postings or _MAX_PAGES is hit.

    `category_type=studentprograms` is the page's own CTA filter; `base_query=
    software` biases toward SWE — KEYWORDS_INCLUDE / KEYWORDS_EXCLUDE in
    filters.py still apply the role-level filtering. tier="big_tech" (same as
    _scrape_amazon) bypasses LOCATIONS_INCLUDE; geography is page-side.

    Expected overlap with _scrape_amazon (different source_prefix → can
    double-email the same posting); accepted, same as google_jobs.
    """
    _SEARCH_URL = (
    ...
    def _on_response(resp):
        if "amazon.jobs" not in resp.url:
            return
        try:
            body = resp.json()
        except Exception:
            return
        # Find a list of job objects: top-level, then one and two levels deep
        # (same heuristic as _scrape_uber._on_response).
        jobs_list = None
        if isinstance(body, list) and body and isinstance(body[0], dict) and "title" in body[0]:
            jobs_list = body
        elif isinstance(body, dict):
    ...
    def _emit(title: str, location: str, job_path: str):
        title = (title or "").strip()
        job_path = (job_path or "").split("?")[0]
        ...
        seen_urls.add(job_url)
        job: Job = {
            "id": make_job_id("amazon_university", "Amazon", title, job_url),
            "title": title,
            "company": "Amazon",
    ...
    for page_idx in range(_MAX_PAGES):
        ...
        try:
            page.goto(
                _SEARCH_URL.format(offset=offset),
                wait_until="load",
                timeout=30000,
            )
        except Exception as exc:
            print(f"[playwright_jobs/amazon_university] WARNING: goto failed at offset {offset}: {exc}")
            break
        # wait_until="load" (not "networkidle") — amazon.jobs holds persistent
        # analytics connections; networkidle never resolves. Give the SPA's XHR
        # time to fire and be captured by the interceptor.
        page.wait_for_timeout(3000)

        # 1) Prefer intercepted JSON (robust against DOM churn).
        for item in captured[captured_before:]:
            try:
                loc = (
                ...
                _emit(
                    ...
                    str(loc),
                    item.get("job_path", "") or item.get("url", "") or item.get("path", ""),
                )
            except Exception:
                continue

        # 2) DOM fallback if interception captured nothing this page.
            ...
            try:
                items = page.evaluate("""() => {
                    ...
                    document.querySelectorAll("a[href*='/jobs/']").forEach(a => {
                        ...
                        const card = a.closest('li, article, div[class*="job"]') || a.parentElement;
                        out.push({
                            title: (a.innerText || '').trim(),
                            href:  href,
                            text:  ((card ? card.innerText : a.innerText) || '').trim(),
                        });
                    });
                    return out;
                }""")
                for it in items:
                    ...
                    lines = [ln.strip() for ln in (it.get("text") or "").splitlines() if ln.strip()]
                    loc = ""
                    if title in lines:
                        i = lines.index(title)
                        if i + 1 < len(lines):
                            loc = lines[i + 1]
                    _emit(title, loc, it.get("href") or "")
            except Exception:
                pass

        # Stop when a page yields no new postings (end of results).
        if len(seen_urls) == before:
            break
    ...
    if not jobs:
        print(
            "[playwright_jobs/amazon_university] WARNING: 0 jobs — verify "
            "amazon.jobs/en/search?category_type=studentprograms still serves "
            "an interceptable jobs JSON or that a[href*='/jobs/'] cards render."
        )
