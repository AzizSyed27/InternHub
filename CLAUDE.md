...
| Amazon University | `https://www.amazon.jobs/en/search.json?business_category[]=studentprograms&normalized_country_code[]=USA&category[]=Software+Development&result_limit=100&offset={n}&sort=recent` | ✅ Working (added 2026-05) — student-programs internships. The user-facing page `amazon.jobs/content/.../internships-for-students` is a marketing page with no postings; its search SPA hits this same `search.json` with the `business_category` / `normalized_country_code` / `category` facets, which **are** honored (verified 2026-05) — unlike `category_type` / `country`, which are silently ignored. Same `_HEADERS`, paginated by `offset` vs integer `hits`. `_scrape_amazon_university()` in `big_tech.py`; gated by `BIG_TECH_ENABLED["amazon_university"]`; uses `make_job_id("amazon", …)` so it dedups against `_scrape_amazon` (one email per posting) |
...
BIG_TECH_ENABLED = {
    "amazon":    True,
    "amazon_university": True,  # student-programs internships via search.json business_category[]=studentprograms + normalized_country_code[]=USA facets (honored, unlike category_type/country); added 2026-05
...
**JSON API scrapers (lower risk):** Greenhouse, Lever, Workday, Amazon, Amazon University — these return structured JSON. A breaking change usually produces an HTTP error or schema mismatch, not silent 0 results. Amazon's `jobs` list field was silently renamed from `hits` in 2026-04 (fixed). Amazon University depends on the `business_category[]=studentprograms` / `normalized_country_code[]=USA` / `category[]=Software Development` facets being honored by `search.json`; if it returns global/non-intern noise, Amazon changed which facets the endpoint respects — re-inspect the studentprograms search SPA's XHR in DevTools.
...
- **Amazon `posted_date` is a human string (2026-05)** — `search.json` returns `posted_date` like `"April 29, 2026"`, with a double space before single-digit days (`"May  7, 2026"`). The old `posted[:10]` slice produced garbage (`"April 29, "`, `"May  7, 20"`) and shipped broken dates in every Amazon email. Both `_scrape_amazon()` and `_scrape_amazon_university()` now parse it via `_parse_amazon_date()` (whitespace-normalize → `strptime("%B %d, %Y")` → ISO `YYYY-MM-DD`, `""` on failure). Do NOT slice Amazon dates. (Apple's `postDateStr` in `_scrape_apple()` still slices, but that Tier-2 scraper is disabled — re-verify its format before enabling.)
...
- **Amazon University internships added 2026-05** — `_scrape_amazon_university()` in `big_tech.py` (Tier 2, stdlib), gated by `BIG_TECH_ENABLED["amazon_university"]`. The user-facing page `amazon.jobs/content/en/career-programs/university/internships-for-students?employment-type[]=Intern&country[]=US` is a marketing landing page with **zero** rendered postings. Investigation initially concluded Playwright was required because `category_type`, `country`, and `employment-type` facets are silently ignored by `search.json` (`category_type=studentprograms` → 10,000 full-time SDE hits). A live smoke test of the studentprograms search SPA's own XHR then revealed it calls the **same `search.json`** with **different facet names** that **are** honored: `business_category[]=studentprograms`, `normalized_country_code[]=USA`, `category[]=Software Development`. So the Playwright approach was scrapped for a plain stdlib JSON scraper (no new deps, robust, returns clean US software interns — verified). Paginates by `offset` vs the integer `hits` total. Uses `make_job_id("amazon", …)` — the **same** `source_prefix` as `_scrape_amazon` — so a posting found by both the keyword search and the student-programs facet dedups to one ID and emails once (better than the originally-planned accepted double-email). `source="BigTech"`, `tier="big_tech"` (location handled API-side, like the other Tier 2 scrapers). **Lesson:** when a careers SPA looks unfilterable, inspect its actual XHR facet param names in DevTools before assuming Playwright — the legacy endpoint may honor differently-named facets.
