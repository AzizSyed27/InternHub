...
| Amazon University | `playwright_jobs.py` | ✅ Added 2026-05: interceptor-first (DOM fallback) scraper on `amazon.jobs/en/search?offset={n}&result_limit=100&category_type=studentprograms&base_query=software`. The user-facing marketing URL `amazon.jobs/content/en/career-programs/university/internships-for-students` renders **no** jobs; legacy `search.json` (big_tech.py) silently ignores the Intern/US/`category_type` facets, so the JS-rendered search SPA is the only source. Response interceptor captures any `amazon.jobs` JSON whose body holds a list of dicts with a `title` (same heuristic walk as Uber); DOM fallback parses `a[href*='/jobs/']` (`/jobs/{id}`) cards. Paginates via `offset`, `_MAX_PAGES=10`, stops on no-new-postings. `source="Amazon"`, `tier="big_tech"` (bypasses location filter; geography is page-side). Overlaps `_scrape_amazon` (different `source_prefix` → can double-email; accepted, same as google_jobs). |
...
PLAYWRIGHT_JOBS_ENABLED = {
    ...
    "amazon_university": True,  # Amazon student-programs internships SPA — search.json ignores Intern/US/category facets; added 2026-05
}
...
SCRAPER_INTERVALS = {
    ...
    "amazon_university": 30,  # same cadence as meta/apple/uber
    "govt_canada":    240,
    "ontario_public": 240,
    "opg":            360,
    "city_toronto":   360,
...
| Amazon University | `playwright_jobs.py` | ✅ | Added 2026-05: interceptor-first, DOM fallback on `amazon.jobs/en/search?...&category_type=studentprograms`. Interceptor (heuristic JSON walk for a list of dicts with `title`) is resilient; DOM fallback selector `a[href*='/jobs/']` may drift. If 0 jobs: (1) open the search URL in DevTools, check the Network tab for the XHR JSON the SPA fetches and confirm it still contains a job array — update the `_on_response` walk if the shape changed; (2) confirm `/jobs/{id}` anchors still render for the DOM fallback. Note: legacy `search.json` ignores the Intern/US/category facets — do NOT "fix" this by reverting to that API. | 2026-05 |
...
- **`PLAYWRIGHT_JOBS_ENABLED` = Meta, YC, Google (careers.google.com), Apple, Uber, intern_list, google_jobs (aggregator), amazon_university** (Tesla disabled 2026-04 — Cloudflare blocking)
...
- **Amazon University internships added 2026-05** — separate from `_scrape_amazon()` in `big_tech.py` (Tier 2). The user-facing page `amazon.jobs/content/en/career-programs/university/internships-for-students?employment-type[]=Intern&country[]=US` is a marketing landing page with **zero** rendered postings; its CTA points at the JS-rendered SPA `amazon.jobs/en/search?...&category_type=studentprograms`. The legacy `search.json` API **silently ignores** `category_type`, `country`, and `employment-type` facets (verified: `category_type=studentprograms` → 10,000 full-time SDE hits), so a stdlib JSON scraper on the official Intern/US filters is impossible — hence a Tier 4 Playwright sub-scraper in `playwright_jobs.py` (`PLAYWRIGHT_JOBS_ENABLED["amazon_university"]`). Interceptor-first (heuristic JSON walk for a list of dicts with `title`, same pattern as Uber), DOM fallback parses `a[href*='/jobs/']` (`/jobs/{id}`) cards. Navigates `wait_until="load"` + 3s (analytics keep `networkidle` from resolving). Paginates by `offset`, `_MAX_PAGES=10`, stops on no-new-postings. `source="Amazon"`, `tier="big_tech"` bypasses `LOCATIONS_INCLUDE` (geography is page-side; same as `_scrape_amazon`). `base_query=software` biases toward SWE; `KEYWORDS_INCLUDE`/`KEYWORDS_EXCLUDE` still apply. Expected overlap with `_scrape_amazon` — different `source_prefix` (`amazon_university` vs `amazon`) means the same posting can dedupe separately and double-email; accepted cost, same as `google_jobs`. `SCRAPER_INTERVALS["amazon_university"] = 30`; folded into the unified `playwright_jobs` interval `min()` in `main.py`.
